...
for igrp = 1:ngrps
    ...
    for iday = 1:ndays
        ...
        if contains(varName, 'fr')
            dataDay{iday} = org_fr(v, varFld);

        elseif contains(varFld, 'bDur')
        ...
        elseif contains(varName, 'psd')
            dataDay{iday} = org_band(v, varFld);

        elseif contains(varName, 'f1f')
            dataDay{iday} = org_f1f(v, varFld);
...
fldName = fieldnames(v(1)); % Renamed to avoid conflict with outer varName
brst = catfields([v(:).(fldName{1})], 'addim', true);
brstData = permute(brst.(brstField), [3, 4, 2, 1]);
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function swvData = org_swv(v, varFld)
% Organizes burstiness data: [mouse x day x unit x 1 x 1]

fldName = fieldnames(v(1)); % Renamed to avoid conflict with outer varName
swv = catfields([v(:).(fldName{1})], 'addim', true);
swvData = permute(swv.(varFld), [3, 4, 2, 1]);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function rippData = org_ripp(v, varName, varFld)
...
if strcmp(varFld, 'rate') ||  strcmp(varFld, 'density')
    % rate of ripple events
    rippTmp = cell(nMice, 1);
    for imouse = 1:nMice
        rippTmp{imouse} = v(imouse).(varName).states.(varFld){stateIdx}(:);
    end
    rippMat = cell2padmat(rippTmp, 2);
    rippData = permute(rippMat, [2, 3, 4, 5, 1]);
    if strcmp(varFld, 'density')
        rippData = rippData / 1000;     % correction of ms
    end

elseif contains(varFld, {'peakPower', 'dur', 'peakFreq', 'maxFreq', 'peakAmp'})
    % ripples parameters
    rippMat = nan(nRipp, nMice);
    for imouse = 1:nMice
        rippTmp = v(imouse).(varName).(varFld);
...
elseif contains(varFld, {'frGain', 'frModulation', 'frPrct', 'Rates'})
    ...
    for iMouse = 1 : nMice
        ...
        elseif contains(varFld, 'ctrlRates')
            spks{iMouse} = ctrlAvg;
        elseif contains(varFld, 'rippRates')
            spks{iMouse} = spksAvg;
        else
            spks{iMouse} = mean(v(iMouse).(varName).spks.su.(varFld), 2, 'omitnan');
        end
    end
