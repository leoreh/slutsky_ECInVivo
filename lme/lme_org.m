function [lme_tbl, lme_cfg] = lme_org(varargin)
...
addOptional(p, 'flg_emg', false, @islogical);
...
if flg_bDur
    bDurCell = lme_load('grppaths', grppaths, 'var_name', var_name, 'var_field', 'bDur', 'vCell', vCell);   
end
    ...
    for imouse = 1:nmice
        for iday = 1:ndays
            ...
            if ~isempty(uCell)
                ...
                units_type = 1;  % Default to unit type 1
            end
            ...
            for istate = 1:nstates
                for ibout = 1:nbouts
                    ...
                    data_tmp = squeeze(dataCell{igrp}(imouse, iday, :, istate, ibout));
                    
                    % Identify valid (non-NaN) values
                    valid_mask = ~isnan(data_tmp);
                    ...
                    % Store data points
                    vec_data(idx_range) = data_tmp(valid_mask);

                    % Create corresponding unit indices and types
                    if length(units_type) > 1
                        % For multiple units, determine which unit each value belongs to
                        [units_idx, ~] = ind2sub(size(data_tmp), find(valid_mask));
                        unit_type = units_type(units_idx);
                    else
                        % For single unit or no units
                        unit_type = repmat(units_type, nValid, 1);
                    end                    

                    % Get bout length if needed
                    if flg_bDur
                        curr_bDur = bDurCell{igrp}(imouse, iday, 1, istate, ibout);
                        vec_bDur(idx_range) = repmat(curr_bDur, nValid, 1);
                    end
                    ...
                    if flg_units
                        for i = 1:nValid
                            ...
                            id_unit(idx_curr + i - 1) = units_idx(curr_idx) + ...
                                1000 * (imouse - 1) + ...
                                10000 * (iday - 1) + ...
                                100000 * (igrp - 1);
                        end
                    else
                        lbl_unit(idx_range) = repmat(string(str_unit{1}), nValid, 1);
...
if flg_bDur && ~strcmp(yName, 'BDur')
    lme_tbl = table(vec_data, lbl_grp, lbl_state, lbl_mouse, lbl_unit, vec_bDur, id_unit, lbl_day,...
...
idxRemove = isnan(lme_tbl.(yName)) | isinf(lme_tbl.(yName));

% Find rows with 'Other' as unit type
idxRemove = idxRemove | lme_tbl.UnitType == 'Other';
...
% Makes BSL the reference and assert the order in str_day
if any(unique(lme_tbl.Day) == categorical({'BSL'}))
    tbl_cats = categories(lme_tbl.Day);
    ordered_cats = str_day(ismember(str_day, tbl_cats));
    lme_tbl.Day = reordercats(lme_tbl.Day, ordered_cats);
end
