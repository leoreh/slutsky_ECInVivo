function [tblRes, hFig] = lme_pr(mdl, vars, varargin)
...
%     % X-Grid (trend line)
%     for iGrp = 1:nGrps
%         tblTmp = table(trends(iGrp).xGrid, 'VariableNames', {varX});
%         tblTmp = tbl_trans(tblTmp, 'template', transParams, 'flgInv', true, ...
%             'varsInc', {varX});
%         trends(iGrp).xGrid = tblTmp.(varX);
%     end
% 
%     % Residuals might also need back transformation (unbiaseD)
% 
% else
    tblRes = tblMdl;
% end
