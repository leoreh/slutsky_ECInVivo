...
clusters2use = input("Enter the clusters to use, in the format [1,2,3]: ");
...
% plot the accepted IEDs and the rejected IEDs on their own clustering:
app_only_accepted = reduct_displayer(ied,{},...
    "reduction_z_norm",false,...
    "reduction_function","t-SNE","tsne_Algorithm",'barneshut',"tsne_dist_fun",'correlation',...
    "cluster_method","dbscan"...
    );
ied_rejected = ied.copy();
ied_rejected = ~ied_rejected.accepted;
app_only_rejected = reduct_displayer(ied_rejected,{},...
    "reduction_z_norm",false,...
    "reduction_function","t-SNE","tsne_Algorithm",'barneshut',"tsne_dist_fun",'correlation',...
    "cluster_method","dbscan"...
    );

% plot the resultant clusters (only accepted) with the result of sleep scoring:
load(fullfile(basepath,basename + ".sleep_states.mat"), "ss");
ied_ss_labels = ied.match_event_state(ss.labels);
cfg = as_loadConfig();
states_names = [cfg.names; {'UNDEFINED'}];
states_names = states_names(unique(ied_ss_labels,"stable")); % reorder to match ied_ss_labels
wave_lb = {'Sleep States', states_names, ied_ss_labels};
app_ss = reduct_displayer(ied, wave_lb,...
    "reduction_z_norm",false,...
    "reduction_function","t-SNE","tsne_Algorithm",'barneshut',"tsne_dist_fun",'correlation',...
    "cluster_method","dbscan"...
    );
