function spks = evt_spks(spkTimes, muTimes, evtTimes, ctrlTimes, peakTime, varargin)
% EVT_SPKS Full spike-modulation analysis around events (one call).
%
%   spks = EVT_SPKS(spkTimes, muTimes, evtTimes, ctrlTimes, peakTime, varargin)
%
%   SUMMARY:
%       Shared spike orchestration for both event pipelines (ripples, ED).
%       The entry point for the spike analysis; combines, in one struct:
%       1. Per-unit scalar modulation stats + per-event population metrics
%          (.events), via evt_spksParams.
...
%           <per-unit scalar stats> - frEvt, frCtrl, cEvt, frZ, frMod, ... (evt_spksParams).
...
%
%   DEPENDENCIES:
%       evt_spksParams, evt_spkPeth, evt_pethNorm.
%
%   HISTORY:
...
% =========================================================================
spks = evt_spksParams(spkTimes, evtTimes, ctrlTimes, peakTime, ...
