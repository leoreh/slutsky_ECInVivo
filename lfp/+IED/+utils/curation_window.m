...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    properties (Access = private)
        ...
        zoom_out_peak     % marker showing current spike
        zoom_out_thr      % lines showing threshold values
%         zoom_out_yrange   % yrange of zoom out data
        zoom_out_marg = 5 % zoom out margins, in secounds

        %%% zoom in props %%%
        zoom_in_line     % line in the zoom in graph
        zoom_in_peak     % marker showing current spike
        zoom_in_thr      % lines showing threshold values
        zoom_in_marg = 0.5 % zoom in margins, in secounds
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    methods (Access = private)
        function [x_data, y_data] = collect_data(app,nSpike,marg)
            % collect what x_data & y_data are relevant to spike.
            % INPUT:
            %   app - base obj.
            %   spike_pos - numeric scalar, idx of spike in detection
            %   marg      - how much to collect around the spike [in sec]
            % OUTPUT:
            %   x_data, y_data - numeric vectors, data to display.
            
            % get pos to work on
            spike_pos = app.ied.pos(nSpike);

            % convert marg to samples
            marg = app.ied.fs * marg;

            % collect window start & end
            win_start = max([1 spike_pos - marg]);
            win_end   =  min([length(app.ied.sig) spike_pos + marg]);
    ...
    methods (Access = private)
        ...
        function IED_curation_UIFigureWindowKeyPress(app, event)
            % Manage key responce - what each key means.
        ...
        function IED_curation_UIFigureWindowButtonUp(app)
            % make axsure toolbar wasn't released, and pass mouse key to
    ...
    methods (Access = public)
        ...
        function app = curation_window(ied)
            ...
            % collect spike info
            [spike_loc_x, spike_loc_y] = app.collect_data(app.ied.last_mark, 0);
            
            % prep zoom out graph
            [x_data, y_data] = app.collect_data(app.ied.last_mark, app.zoom_out_marg);
            app.zoom_out_line = plot(app.zoom_out_axe ,x_data, y_data);
            app.zoom_out_peak = scatter(app.zoom_out_axe, spike_loc_x, spike_loc_y, 'r*');
            ...
            app.zoom_in_peak = scatter(app.zoom_in_axe, spike_loc_x, spike_loc_y, 'r*');
