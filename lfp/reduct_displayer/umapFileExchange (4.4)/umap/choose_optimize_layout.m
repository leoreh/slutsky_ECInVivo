function [embedding, method] = choose_optimize_layout(head_embedding,...
    tail_embedding, head, tail, n_bouts, n_vertices,bouts_per_sample,...
    a, b, gamma, initial_alpha, negative_sample_rate, verbose, method,...
    progress_callback, bout_reports, random_state, min_dist, ...
...
if strcmpi(method, 'Mex')
    try
        ...
        if ~exist(exe, 'file')
            method='Java';
        elseif verbose
            if reportMexProgress(head_embedding, [1 n_bouts])==0
                embedding=[];
                return;
            end
            if isequal(head_embedding, tail_embedding)
                ...
                embedding=mexStochasticGradientDescent(head_embedding, [],...
                    uint32(head), uint32(tail), uint32(n_bouts), ...
                    uint32(n_vertices), bouts_per_sample, a, b, gamma,...
                    initial_alpha, negative_sample_rate, ...
                    uint32(~random_state), ...
                    @(data, bouts)reportMexProgress(data, bouts), ...
                    uint32(sgd_tasks));
            else
                embedding=mexStochasticGradientDescent(head_embedding, ...
                    tail_embedding, uint32(head), uint32(tail), ...
                    uint32(n_bouts),  uint32(n_vertices), bouts_per_sample, ...
...
if strcmpi(method, 'C++')
    try
        ...
        embedding=StochasticGradientDescent.Go(inFile, ...
            ...
            head, tail, n_bouts, n_vertices, bouts_per_sample, a, b, gamma, ...
...
if strcmpi(method, 'Java')
    initJava;
    try
        N=size(bouts_per_sample, 1);
        if TEST_CROSS_ENTROPY
            weights = ones(N,1)./bouts_per_sample; %We probably should have passed in weights to this instead...
        end 
        javaObject=edu.stanford.facs.swing.StochasticGradientDescent(...
            head_embedding, tail_embedding,head, tail, n_bouts, ...
        ...
        if bout_reports>0
            javaObject.setReports(bout_reports);
        end
        if ~reportJavaProgress
            embedding=[];
            return;
        end
        while ~javaObject.nextBouts
            if ~reportJavaProgress
                embedding=[];
                return;
            end
...
if strcmpi(method, 'C')
    try
        embedding = optimize_layout_mex(single(head_embedding), ...
            ...
            int32(n_bouts), int32(n_vertices), ...
            single(bouts_per_sample), single(a), single(b), ...
...
elseif strcmpi(method, 'MATLAB experimental 2')
    ...
    function ok=reportMexProgress(data, bouts)
        try
            if isequal('function_handle', class(progress_callback))
                progressObj.getBoutsDone=double(bouts(1));
                progressObj.getBoutsToDo=double(bouts(2));
    ...
    function wantsToContinue=reportJavaProgress        
        if isequal('function_handle', class(progress_callback))
            ...
            if verbose
                done=javaObject.getBoutsDone-1;
                toDo=javaObject.getBoutsToDo;
