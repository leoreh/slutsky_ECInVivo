...
% basepaths2vars, and defines key parameters for the analysis.
...
% Vectorized "Linear Scavenger" Control Allocator
maxDist = 4 * 60 * 60;
ctrlTimes = nan(nRipples, 2);
padding = 0.1;

% Pre-calculate all available free blocks once (O(N) operation)
% intervalsCtrl already contains valid segments (duration > max(rippDur))
...
for iEvent = 1:nRipples
    ...
    needed = rippDur(iEvent);

    % Find first valid free block
    while currentFreeIdx <= nFree

        blockStart = intervalsCtrl(currentFreeIdx, 1);
        blockEnd = intervalsCtrl(currentFreeIdx, 2);

        % Calculate earliest possible start time for this event
        % Start after previous control + padding
        t_start = max(blockStart, lastEndTime + padding);

        % Check if this fits in current block
        if t_start + needed <= blockEnd

            % Check maxDist constraint (too far in past). If the *block
            % start* is too old, we discard the BLOCK.
            if blockStart < rippPeakTime(iEvent) - maxDist
                currentFreeIdx = currentFreeIdx + 1;
                continue;
            end

            % If we survived the checks, we found our spot!
            ctrlTimes(iEvent, :) = [t_start, t_start + needed];
            lastEndTime = t_start + needed;
            break; % Move to next event

        else
            % Does not fit in current block, move to next
            currentFreeIdx = currentFreeIdx + 1;
        end
    end

    if currentFreeIdx > nFree
        warning('Ran out of control intervals at event %d/%d', iEvent, nRipples);
        break;
    end
end
...
% Define search windows for each event
winStart = eventTimes + mapDur(1);
winEnd = eventTimes + mapDur(2);

% Find start/end indices of spikes falling into each event's window. We
% treat spikeTimes as "edges" to find where the window boundaries fall. The
% index returned corresponds to the first spike >= boundary.

% Add dummy edges to handle boundary conditions safely
searchEdges = [-inf; spikeTimes; inf];

% Find indices of first spike >= winStart
startIdx = discretize(winStart, searchEdges);

% Find indices of first spike > winEnd (so we stop before this)
[~, endBin] = histc(winEnd, searchEdges);
endIdx = endBin - 1;

% Identify events that actually have spikes in their window
validEvents = find(endIdx >= startIdx);
nValid = length(validEvents);

if nValid == 0
    return;
end

% Create a ragged array using Run-Length Encoding logic
spkInEvent = endIdx(validEvents) - startIdx(validEvents) + 1;

% Create event ID vector: repeated event indices
eventIds = repelem(validEvents, spkInEvent);

% Create global spike index vector

% Pre-allocate index vector
totalSpikes = sum(spkInEvent);
spkEventIdx = zeros(totalSpikes, 1);
currentPos = 1;

% This loop is strictly for index generation and is very fast (millions of iters/sec)
for iEvent = 1:nValid
    eventIdx = validEvents(iEvent);
    nSpks = spkInEvent(iEvent);
    spkEventIdx(currentPos : currentPos + nSpks - 1) = startIdx(eventIdx) : endIdx(eventIdx);
    currentPos = currentPos + nSpks;
end

% Extract all relevant spikes
spkEventTimes = spikeTimes(spkEventIdx);

% Calculate relative times
relTimes = spkEventTimes - eventTimes(eventIds);

% Bin the relative times
edges = linspace(mapDur(1), mapDur(2), nBinsMap + 1);
bins = discretize(relTimes, edges);

% Accumulate counts into the matrix
% Rows = Events, Cols = Bins
% Note: size(syncMap) is nEvents x nBinsMap
syncMap = accumarray([eventIds, bins], 1, [nEvents, nBinsMap]);
