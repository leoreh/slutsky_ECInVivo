...
%   2. An index indicating which ripples occurred during each state.
%   3. Stores these results in the input 'ripp' structure.
%   4. Optionally plots the overall ripple rate overlaid with state-specific 
%      rates and a pie chart showing the distribution of ripples across states.
%   5. Optionally saves the updated 'ripp' structure.
%
% INPUT:
...
%                                             edges for state rate calc [s].
%                       ripp.states.tstamps: {nStates x 1} cell array, 
%                                            timestamps for state rate calc [s].
...
binsizeRate = ripp.info.binsizeRate;
    ...
    if ~isempty(stateBouts)
        ...
        [ripp.states.rate{iState}, ripp.states.binedges{iState}, ...
         ripp.states.tstamps{iState}] = ...
            times2rate(peakTime, 'binsize', binsizeRate, ...
...
if flgGraphics
    ...
    % Subplot 2: Pie Chart of Ripple Distribution across States
    sb2 = subplot(1, 4, [3, 4]);
