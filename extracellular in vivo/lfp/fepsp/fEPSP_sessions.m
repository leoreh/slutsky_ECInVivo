...
basepath = 'I:\Data\Processed\lh69';
...
if forceA
    for i = 1 : nsessions
        ...
        load([basename '.fepsp.mat'])
        dt = round(2 / 1000 * 1250);
        [~, midx1] = min(abs(fepsp.t - 0));
        [~, midx2] = min(abs(fepsp.t - 30));
        wvwin = [midx1 + dt, midx2];
        fepsp.wvsnip = fepsp.wvavg(:, :, wvwin(1) : wvwin(2));
        save([basename '.fepsp.mat'], 'fepsp');
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% rearrange data
...
ss = [1 : nsessions];
p = 1;
clr = ['kkkrrrr'];        % must be sorted (i.e. g before k before r)
fs = 1250;
if p
    fh = figure;
    subplot(2, 1, 1)
