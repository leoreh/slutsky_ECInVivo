function [net] = AccuSleep_train_nStates(fileList, SR, epochLen, epochs, imageLocation)
...
%       {1,2,3} are ignored during training (CHANGED LH FEB21).
...
for i = 1:nFiles
    ...
    if nstates == 3
        weights = [.1 .35 .55]; % rem wake nrem DROWSY      % ----------- %
    elseif nstates == 4
        weights = [.1 .3 .5 .1];
    else
        error('weights and states uncompatible')
    end
    ...
    for j = (pad+1):(length(processedEMG)-pad)
        ...
        if data.c.labels(j) > 0 && data.c.labels(j) <= nstates  % ----------- %
...
layers = [
    ...
    convolution2dLayer(nstates,8,'Padding','same')             % ----------- %
    batchNormalizationLayer
    reluLayer
    maxPooling2dLayer(2,'Stride',2)
    convolution2dLayer(nstates,16,'Padding','same')            % ----------- %
    batchNormalizationLayer
    reluLayer
    maxPooling2dLayer(2,'Stride',2)
    convolution2dLayer(nstates,32,'Padding','same')            % ----------- %
    batchNormalizationLayer
    reluLayer
