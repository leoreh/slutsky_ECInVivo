...
% check if config file has any issues
...
% get number of states and check if all labels are outside the range 1:n_states
config = load_config_data();
if ischar(config)
    disptext(handles, config);
    lockInputs(handles,0);
    return
end
n_states = length(config.cfg_names);
if all(labels > n_states | labels < 1)
    animateBoxes(getappdata(handles.D,'outputIndicators'),0);
    disptext(handles, ['ERROR: At least some labels must be in the range 1:',...
        num2str(n_states),'.']);
...
% get config data and number of states
config = load_config_data();
if ischar(config)
    disptext(handles, config);
    lockInputs(handles,0);
...
for i = 1:length(allRecordings)
    ...
    if get(handles.overwriteBox,'Value') && exist(allRecordings{i}.labelpath, 'file')
        ...
        if isfield(d, 'labels')
            ...
            userLabels(userLabels > n_states) = 0;
