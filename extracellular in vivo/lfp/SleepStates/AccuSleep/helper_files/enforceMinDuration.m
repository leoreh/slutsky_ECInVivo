...
% Check the inputs
if length(limits) ~= n_states
    error(['2nd argument must be a vector of length ',num2str(n_states)]);
end
...
while 1
    ...
    for i = 1:n_states % for each brain state
        for j = setxor(i,1:n_states) % for each possible surrounding state
