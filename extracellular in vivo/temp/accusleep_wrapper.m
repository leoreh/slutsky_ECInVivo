...
elseif cleanRec == 2
    ...
    badtimes = zeros(floor(recDur / cleanRes), 1);
    badtimes(zscore(mean(zscore(s), 2)) > artifactThr) = 1;
else
    badtimes = [];
end
...
if ~isempty(badtimes)
rmtimes = [interp1([1 : length(badtimes)] * cleanRes, badtimes, tstamps_sig,...
    'linear', 'extrap')]';
else
...
for i = 1 : size(badEpochs, 1)
    labels = [labels(1 : badEpochs(i, 1) - 1);...
        ones(diff(badEpochs(i, :)), 1) * 4;...
    labels(badEpochs(i, 1) : end)];
end
