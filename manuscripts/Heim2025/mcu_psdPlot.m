...
addOptional(p, 'type', 'freq', @(x) ismember(x, {'freq', 'heat'}));
...
if strcmpi(plotType, 'heat')
    ...
    for igrp = 1 : 2
        ...
        plot_stdShade('dataMat', psd_data{igrp}, 'xVal', psd_cfg.faxis,...
            'axh', axh, 'clr', clr, 'alpha', 0.5)
    end

    % formatting
    if flg_log
        set(axh, 'YScale', 'log', 'XScale', 'log')
    end
    xlabel('Frequency [Hz]')
    ylabel('Power')
    ...
    ymax = max(mean(allData, 2));
    ymin = min(mean(allData, 2));
