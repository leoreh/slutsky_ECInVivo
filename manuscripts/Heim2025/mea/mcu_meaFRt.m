...
% Correct timevector based on realization that Max data skips every other
% hour after perturbation stops (01-nov-25)
tOrig = t;
t(t > 0) = t(t > 0) * 2;
...
for iGrp = 1 : 2
    ...
    frMat = (fr ./ frBsl) * 100;
    % frMat = (log(fr + 1e-6) - log(frBsl)) * 100;
...
for iPrc = 1 : nPrc
    ...
    elseif normType == 2
        frMat = fr ./ mean(frBslPerc, 'omitnan') * 100;
    else
        frMat = fr;
    end
