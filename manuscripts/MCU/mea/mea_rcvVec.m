...
% Model dBrst and Extract Residuals
frml = 'dBrst ~ (pBspk + fr) * Group + (1|Name)';
[lmeMdl, ~, ~, ~] = lme_analyse(tbl, frml, 'dist', 'normal', 'verbose', false);
tbl.R_burst = residuals(lmeMdl, 'ResidualType', 'Pearson');

% Model dSngl and Extract Residuals
frml = 'dSngl ~ (pBspk + fr) * Group + (1|Name)';
[lmeMdl, ~, ~, ~] = lme_analyse(tbl, frml, 'dist', 'normal', 'verbose', false);
tbl.R_single = residuals(lmeMdl, 'ResidualType', 'Pearson');

% Correlation Analysis
[rRes, pRes] = corr(tbl.R_burst, tbl.R_single, 'Rows', 'complete');
frml = 'R_single ~ R_burst * Group + (1|Name)';
[lmeMdl, ~, ~, ~] = lme_analyse(tbl, frml, 'dist', 'normal', 'verbose', false);
...
for iGrp = 1:length(groups)
    ...
    xData = tbl.R_burst(idx);
    yData = tbl.R_single(idx);
    
    % Scatter
    scatter(xData, yData, 30, cols(iGrp,:), ...
        'filled', 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', 0.3, ...
    ...
    plot(xRange, yFit, 'Color', cols(iGrp,:), 'LineWidth', 2, ...
        'DisplayName', sprintf('%s Fit', string(groups(iGrp))));
end

xlabel('dBrst Residuals');
ylabel('dSngl Residuals');
title('Residual Analysis');
grid on;
axis square;

% Set Limits (Symmetric)
maxVal = max(abs([tbl.R_burst; tbl.R_single])) * 1.1;
xlim([-maxVal maxVal]);
ylim([-maxVal maxVal]);


% Reference Lines (Zero axes)
xline(0, '--k', 'Alpha', 1, 'HandleVisibility', 'off');
yline(0, '--k', 'Alpha', 1, 'HandleVisibility', 'off');
