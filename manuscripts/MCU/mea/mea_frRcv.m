...
%       uGood       - (logical) Vector indicating valid "good" units. This subset
%                     is used to decide if the bias correction constant 'c'
%                     should be added to all units. {Default: all true}
...
%                       .idxTrough  - Index used for trough.
%
%   NOTES:
...
% Constants
c = 1 / 3600;           % 1 spike per hour
thrPert = 1;            % Threshold for significant perturbation (log2 fold)
thrRcv = 0.67;          % Threshold for recovery (fraction of range)
...
% Baseline Rate
% -------------------------------------------------------------------------
% Window: 1 hour ending 5 min before perturbation
...
% Steady State Rate
% -------------------------------------------------------------------------
% Window: 1 hour ending 5 min before end of recording
...
% Trough Rate
% -------------------------------------------------------------------------
% Window: 10 min surrounding the provided trough index
troughStart = max(1, idxTrough - winHalf);
troughEnd = min(length(t), idxTrough + winHalf);
...
% Perturbed Units
% Significant perturbation AND "dead-zone" lower then frBsl
frDead = mean(frMat(:, idxPert : idxTrough), 2, 'omitnan');
uPert = pertDepth >= thrPert & frDead < frBsl;

% Recoverd Units
% Significant perturbation AND reached 67% of recovery range
uRcv = uPert & (frSs >= frTrough + thrRcv * (frBsl - frTrough));
uRcv(~uGood) = false; % Valid only for good units
