...
% based on Vinck 2012 and bz_GenSpikeLFPCoupling.
...
window = max([5, 1]);
mtspec_params.pad = 0;
mtspec_params.Fs = 1250;
mtspec_params.fpass = [20 60];
mtspec_params.tapers = [10 19];
mtspec_params.err = [1 0.05];
mtspec_params.trialave = 1;

[C,phi,S12,S1,S2,f,zerosp,confC,phistd]=...
coherencycpt(sig, stimes{3}', mtspec_params, 1);
fh = figure;
plot(f, C)
...
%     stimes = stimes(units.idx(1, :));
    ...
    [SpikeLFPCoupling] = bz_GenSpikeLFPCoupling(spikes, lfp,...
        'spikeLim', 500000, 'frange', [6 12], 'nfreqs', [1],...
        'sorttype', 'rate', 'saveMat', true, 'int', winCalc);
