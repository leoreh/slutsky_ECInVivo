...
parfor iUnit = 1:nUnits
    ...
    [c, m] = transfer_fn(st, single(t), edges, params, true, iUnit);
...
mito = accumarray(binIdx(valid)', M(valid)', [nBins 1], @max)';

% Plotting
if flgPlot
    % Calculate tBins for plotting
    tBins = edges(1:end-1) + diff(edges)/2;
    plot_spk2ca(t, st, C, M, tBins, cyto, mito, uid, Kd);
end
...
function plot_spk2ca(t, st, C, J, tBins, cyto, mito, uid, Kd)
% PLOT_MCA_UNIT Plot high-res dynamics for a single unit.

% HARD CODE LIMIT
winLim = [0, Inf] * 3600;

% Filter Data (High Res)
idx = t >= winLim(1) & t <= winLim(2);
if ~any(idx)
    return;
end
...
% Filter Spikes
st = st(st >= winLim(1) & st <= winLim(2));

% Filter Binned Data
idxBin = tBins >= winLim(1) & tBins <= winLim(2);
tBins = tBins(idxBin);
cyto = cyto(idxBin);
mito = mito(idxBin);
...
ax4 = nexttile(hTile);
yyaxis(ax4, 'left');
plot(ax4, tBins, cyto, 'b-', 'LineWidth', 1);
ylabel(ax4, 'Cyto (Mean)');

yyaxis(ax4, 'right');
plot(ax4, tBins, mito, 'r-', 'LineWidth', 1);
ylabel(ax4, 'Mito (Sum)');

title(ax4, 'Binned Dynamics');
grid(ax4, 'on');
