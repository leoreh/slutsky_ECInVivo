...
%                     .mitoIn   : (nUnits x nBins) Integrated Flux (Psi_mCa), summed * dt.
%                     .mito     : (nUnits x nBins) Matrix Calcium (M), averaged.
...
parfor iUnit = 1:nUnits
    ...
    [c, mIn, m] = transfer_fn(st, single(t), edges, params, false, iUnit);
...
function [cyto, mitoIn, mito] = transfer_fn(st, t, edges, params, flgPlot, uid)
...
mitoIn = accumarray(binIdx(valid)', J(valid)', [nBins 1], @sum)' * dt;
mito = accumarray(binIdx(valid)', M(valid)', [nBins 1], @(x) x(end))';

% Plotting
if flgPlot
    plot_spk2ca(t, st, C, J, M, uid, Kd);
end
...
function plot_spk2ca(t, st, C, J, M, uid, Kd)
...
% Mito Conc
ax4 = nexttile(hTile);
plot(ax4, t, M, 'm-', 'LineWidth', 1.5);
title(ax4, 'Matrix Calcium (M)');
xlabel(ax4, 'Time (s)');
ylabel(ax4, 'Conc.');
grid(ax4, 'on');
