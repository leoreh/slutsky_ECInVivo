function stg = calc_stgs(varargin)
...
%   mancur      logical. manual curate the results or not {true}. 
...
% 10 may 25         separated graphics, changed to calc_stg
...
addOptional(p, 'mancur', true, @islogical);
...
roi_ms = [1, 7];         % region of interest for mono synaptic connections [ms]
alfa = 0.0001;           % significance level (consider differentiating for E and I)
refThr = 1;
...
if ischar(saveVar)
    stgfile = fullfile(basepath, [basename, '.stg', saveVar, '.mat']);
else
    stgfile = fullfile(basepath, [basename, '.stg.mat']);
end
if exist(stgfile) && ~forceA
    load(stgfile)
    return
end
...
[eIdx(:, 1), eIdx(:, 2)] = find(eSig);
[iIdx(:, 1), iIdx(:, 2)] = find(iSig);
stg.synIdx = [[eIdx; iIdx], [ones(length(eIdx), 1); -ones(length(iIdx), 1)]];
stg.eBins = eBins;
stg.iBins = iBins;
stg.eSig = eSig;
stg.iSig = iSig;
...
stg.info.runtime = datetime(now, 'ConvertFrom', 'datenum');
stg.info.basename = basename;
stg.info.winCalc = winCalc;
stg.info.ccThr = ccThr;
stg.info.refThr = refThr;
stg.info.stgThr = stgThr;
stg.info.roi_ms = roi_ms;   
stg.info.roi_binIdx = roi_binIdx;     
stg.info.alfa = alfa;
stg.info.pred = pred;
stg.info.minConsecBins = minConsecBins;
...
if mancur
    mancur_stg('stg', stg, 'basepath', basepath)
end
